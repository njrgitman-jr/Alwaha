...
export default function AccessoryCard({
  ...
  return (
    ...
    >
      <HoverZoomImage
        ...
        fallback={
          <div className="aspect-square rounded-xl tile-bg flex items-center justify-center mb-3">
            <div className="text-6xl drop-shadow group-hover:scale-110 transition-transform duration-300">
              🔧
            </div>
          </div>
        }
      />
      {a.freeShipping && (
        <span className="chip-accent text-[10px] self-start mb-1">
          🚚 توصيل مجاني
        </span>
      )}
      <h3 className="font-bold text-sm leading-snug text-ink-900 line-clamp-2 min-h-[2.5rem] group-hover:text-brand-700 transition-colors">
        {a.name}
      </h3>
      {a.rating != null && (
        <div className="mt-1">
          <StarRating value={a.rating} count={a.ratingCount} />
        </div>
      )}
      {a.note && (
        <p className="text-xs text-ink-500 mt-1 line-clamp-2">{a.note}</p>
      )}
